...
/**
 * Classes the current user can mark attendance for.
 */
...
/**
 * Fetch students + existing attendance for a date, for ONE class.
 * @param {string} date - YYYY-MM-DD
 * @param {string} className
 */
export const getMarkPage = async (date, className) => {
  const { data } = await api.get('/attendance/mark', { params: { date, class: className } });
  return data;
};
 ...
 */
export const submitAttendance = async (date, className, entries) => {
  const { data } = await api.post('/attendance/submit', { date, class: className, entries });
  return data;
};
