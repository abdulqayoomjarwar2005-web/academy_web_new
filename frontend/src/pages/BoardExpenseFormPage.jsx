...
import { BOARD_EXPENSE_CATEGORIES, createBoardExpense, getBoardExpense, updateBoardExpense } from '../utils/boardApi';
...
const BoardExpenseFormPage = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const isEdit = Boolean(id);

  const [expenseDate, setExpenseDate] = useState(today());
  const [category, setCategory] = useState('examination_fee');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await updateBoardExpense(id, payload);
      } else {
        await createBoardExpense(payload);
      }
      navigate('/board/expenses');
    } catch (err) {
  ...
  return (
    <DashboardLayout title={isEdit ? 'Edit Board Expense' : 'Add Board Expense'}>
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="max-w-md space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              type="button"
              onClick={() => navigate('/board/expenses')}
...
export default BoardExpenseFormPage;
